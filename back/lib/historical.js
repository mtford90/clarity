...
/**
 * An event emitter that will emit stats at a given rate.
...
var StatsMonitor = function (sshPool, rate) {
    ...
    this.start = function () {
        ...
        self.intervalIdentifiers = _.map([swapUsed, load], _.partial(setInterval, [_, self.rate]));
    };

    this.stop = function () {
        _.map(self.intervalIdentifiers, setInterval);
    };
